...
# Page setup
st.set_page_config(page_title="CORA Dashboard", page_icon="🎯", layout="wide")
...
# Title
st.markdown('<p class="main-header">🎯 CORA Lead Generation Dashboard</p>', unsafe_allow_html=True)
...
if not df.empty:
    ...
    with col1:
        search = st.text_input("🔍 Search by name, email, or organization", "")
    with col2:
    ...
    # Display leads table
    st.subheader(f"📋 Leads ({len(filtered_df)} total)")
    ...
    st.markdown("---")
    st.subheader("🆕 Most Recent Leads")
